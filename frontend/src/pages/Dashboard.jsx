        ...
        {Object.entries(providers).map(([name, info]) => {
          ...
          return (
            ...
            >
              ...
              {m.short}
              {info.calls > 0 && (
                <span className="chip-count">{info.calls}</span>
              )}
            </span>
          );
        })}
...
const ChartBlock = ({ chart, idx, theme }) => {
  ...
  const tip = {
    ...
    boxShadow: isDark
      ...
      : "0 8px 32px rgba(100,65,180,0.15)",
    padding: "10px 14px",
  };
  ...
  return (
    <div
      className="chart-card animate-fade-up"
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "ai",
          text:
            "Error: " + (e.response?.data?.detail || "Could not get answer."),
          error: true,
        },
      ]);
    }
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-file-info">
          ...
          <div>
            <div className="sidebar-file-name">
              {fileMeta?.filename || "File"}
            </div>
            <div className="sidebar-file-meta">
              {fileMeta?.rows > 0 ? `${fileMeta.rows} rows` : ""} ·{" "}
              {fileMeta?.chunks} chunks
            </div>
          </div>
        </div>
        ...
        {summary.top_insight && (
          <div className="sidebar-insight">
            <div className="sidebar-insight-label">
              <Sparkles size={11} />
              Top insight
            </div>
      ...
      <main className="main-content">
          ...
          <div className="dash-header-row">
            <div>
              <h1 className="dash-title">
                {summary.headline || "Intelligence Dashboard"}
              </h1>
              <p className="dash-sub">
                Powered by MoGenAI · {fileMeta?.filename}
              </p>
            </div>
            <button
          ...
          <div className="metrics-grid">
            {metrics.map((m, i) => (
              ...
              >
                ...
                <div className={`metric-trend trend-${m.trend}`}>
        ...
        {charts.length > 0 && (
          <div className="charts-grid">
            {charts.map((c, i) => (
              <ChartBlock key={i} chart={c} idx={i} theme={theme} />
            ))}
          </div>
        )}
          ...
          <div className="chat-messages">
            {messages.length === 0 && (
              <div className="chat-empty">
                <BrainIcon />
                <p>
                  Ask any business question. Every answer is cited from your
                  data.
                </p>
              </div>
            )}
            {messages.map((msg, i) => (
              <div
                key={i}
                className={`chat-msg msg-${msg.role} animate-fade-in`}
              >
                {msg.role === "ai" && (
                ...
                <div className={`msg-bubble ${msg.error ? "msg-error" : ""}`}>
                  ...
                  {msg.sources?.length > 0 && (
                    <div className="msg-sources">
                      ...
                      {msg.provider && (
                        <span className="source-chip provider-source-chip">
                          via {PROVIDER_META[msg.provider]?.short}
                        </span>
                      )}
                    </div>
                  )}
                ...
                {msg.role === "user" && (
                  <div className="msg-avatar user-avatar">M</div>
                )}
              </div>
            ))}
            {loading && (
